<template>
  <div class="d-flex align-center" style="gap: 8px">
    <v-btn
      variant="elevated"
      id="campaign-import-hero-from-campaign"
      rounded
      @click="openModal"
    >
  ...
  <v-dialog v-model="visible" max-width="500">
    <v-card>
      <v-card-title class="text-center">
        <span v-if="!selectedSourceCampaignId">{{
          t("Select Source Campaign")
        }}</span>
      ...
      <v-card-text>
        <div v-if="!selectedSourceCampaignId">
          <template v-if="availableSourceCampaigns.length > 0">
            <v-list lines="one">
              <v-list-item
                v-for="campaign_item in availableSourceCampaigns"
                :key="campaign_item.campaignId"
                @click="selectSourceCampaign(campaign_item.campaignId)"
              >
                <template v-slot:prepend>
                  <div
                    class="d-flex align-center"
                    style="min-width: 60px; height: 36px"
                  >
                    <v-img
                      v-if="campaign_item.campaign === 'core'"
                    ...
                    />
                    <v-img
                      v-else-if="campaign_item.campaign === 'apocalypse'"
                      class="campaign-list-logo"
                    ...
                    />
                    <v-img
                      v-else-if="campaign_item.campaign === 'awakenings'"
                      class="campaign-list-logo"
                    ...
                    />
                    <v-img
                      v-else-if="campaign_item.campaign === 'underkeep'"
                      class="campaign-list-logo"
          ...
          <p v-else class="text-center">
            {{ t("No other campaigns available to import from.") }}
          </p>
        </div>
        ...
        <div v-else>
          ...
          <template v-if="heroesInSelectedSourceCampaign.length > 0">
            <v-list lines="one">
              ...
              >
                <v-img
                  v-if="
                    getStaticHeroData(heroInstance.heroId)?.images?.trackerimage
                  "
                  :src="
                    getStaticHeroData(heroInstance.heroId)?.images.trackerimage
                  "
                  :alt="
                    getStaticHeroData(heroInstance.heroId)?.name ||
                    heroInstance.heroId
                  "
          ...
          <template v-else>
            <p class="text-center mt-4">
              {{ t("No new heroes to import from this campaign.") }}
            </p>
          </template>
        </div>
      </v-card-text>
...
const availableSourceCampaigns = computed(() => {
  ...
  const filteredCampaigns = allCampaigns.filter(
    (campaign) => campaign.campaignId !== props.campaignId,
  );
...
const heroesInSelectedSourceCampaign = computed(() => {
  ...
  const sourceHeroes = heroStore.findAllInCampaign(
    selectedSourceCampaignId.value,
  );
  return sourceHeroes.filter(
    (hero) => !heroStore.hasInCampaign(hero.heroId, props.campaignId),
  );
});
...
function importHeroToCurrentCampaign(heroToImport: Hero) {
  if (!selectedSourceCampaignId.value) {
    toast.add({
      severity: "warn",
      summary: t("Warning"),
      detail: t("No source campaign selected."),
      life: 3000,
    });
    return;
  }
  ...
  const importSuccessful = heroStore.importAndCloneHeroToCampaign(
    heroToImport,
    props.campaignId,
  );
  if (importSuccessful) {
    ...
    toast.add({
      ...
      detail: t(
        "Hero could not be imported. They might already be in the target campaign.",
      ),
      life: 3000,
    });
  }
