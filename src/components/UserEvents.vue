          ...
          <v-card color="white" class="pt-0 event-card" @click="openDialog(event)">
            <v-row no-gutters>
              ...
              <v-col cols="8" sm="10" class="pt-2 pl-2">
                ...
                <p class="text-caption text-truncate mt-1"> <v-icon color="red">mdi-map-marker</v-icon> {{ event.location }}
                </p>
                <p class="text-caption font-weight-medium mb-1 mt-1">
                  Status: <span class="font-weight-bold">{{ event.status || 'No status' }}</span>
                </p>
...
const fetchPlayerEvents = () => {
  axios.get("events/my_events/player", {
    params: {
      player_fk: appUser,
      limit: 30,
      offset: 0,
    ...
    .then((response) => {
      ...
      events.value = fetched.map(event => ({
        id: event.events_pk,
        ...
        eventdesc: "",
        shopdesc: "",
        eventseats: event.seats_number,
        shopname: event.store_name,
    ...
    .catch((error) => {
      console.error("Erro ao buscar eventos do jogador:", error);
    });
};
