<template>
    <v-main class="lobby-page bg-black h-screen w-screen overflow-hidden">
        ...
        <div class="responsive-container d-flex flex-column fill-height position-relative" style="z-index: 1">
            <div class="px-4 pt-4 pb-2 flex-shrink-0 w-100">
                <div class="d-flex align-center justify-space-between mb-2">
                    ...
                    <div class="d-flex flex-column align-center">
                        ...
                        <span v-if="eventDateObj.day" class="text-caption text-grey">
                            {{ eventDateObj.month }} {{ eventDateObj.day }} -
                            {{ eventDateObj.time }}
                        </span>
                ...
                <div v-if="selectedCampaign" class="text-center mt-2">
                    <v-chip color="purple-accent-2" variant="flat" :closable="isLeader" @click:close="clearCampaign"
            ...
            <div class="flex-grow-1 w-100 px-4 overflow-y-auto d-flex flex-column justify-center">
                <v-row dense class="w-100 ma-0 justify-center">
                    <v-col cols="6" v-for="(slot, index) in lobbySlots" :key="index" class="pa-3 d-flex justify-center">
                        <v-card class="player-slot-card d-flex flex-column align-center justify-center elevation-10"
                            :color="slot.player ? '#1e1e1e' : 'rgba(255,255,255,0.05)'"
                            :variant="slot.player ? 'elevated' : 'outlined'" rounded="xl"
                            @click="handleSlotClick(index)" v-ripple="!!slot.player" style="
                width: 100%;
                max-width: 220px;
                aspect-ratio: 0.76;
                position: relative;
              ...
              ">
                            ...
                            <template v-else>
                                <div class="player-overlay-header">
                                    ...
                                    <v-icon v-if="index === 0" color="amber" size="small" class="ml-1"
                                        title="Drunagor Master">
                                        mdi-crown
                                    </v-icon>
                                </div>

                                <template v-if="slot.hero">
                                    <v-img :src="slot.hero.image" cover class="w-100 h-100"
                                        position="top center"></v-img>
                                </template>

                                <template v-else>
                                    <div
                                        class="d-flex flex-column align-center justify-center fill-height w-100 bg-grey-darken-4 border-dashed pt-6">
                                        <v-progress-circular v-if="slot.loadingHero" indeterminate color="white"
                                            size="24"></v-progress-circular>
                                        <v-icon v-else color="grey-lighten-1" size="x-large"
                                            class="mb-2">mdi-shield-account-outline</v-icon>
                                        <span
                                            class="text-caption text-grey-lighten-1 font-weight-bold text-uppercase mt-2">
            ...
            <div class="flex-shrink-0 pa-6 w-100 bg-grey-darken-4 elevation-24 rounded-t-xl z-index-10">
                <v-btn block :color="mainButtonConfig.color" height="64"
                    class="font-weight-bold text-white text-h6 rounded-lg elevation-4" :loading="loadingStart"
                    @click="handleMainAction" :disabled="mainButtonConfig.disabled">
        ...
        <v-overlay v-model="loadingStart" class="align-center justify-center" persistent opacity="0.9" scrim="black">
            <div class="d-flex flex-column align-center justify-center text-center">
                <v-img src="@/assets/logo/underkeep.png" width="200" class="mb-6"></v-img>
                <v-progress-circular indeterminate color="amber-accent-4" size="64" width="6"
                ...
                <p class="text-grey-lighten-1 text-body-2">
                    Preparing the dungeon for your arrival.
                </p>
            </div>
        </v-overlay>
...
const fetchTablePlayers = async () => {
    ...
    try {
        ...
        // Se não é líder, verifica se já entrou no jogo
        if (!isLeader.value) {
