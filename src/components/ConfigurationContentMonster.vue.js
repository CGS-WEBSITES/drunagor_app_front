...
const toggleSelection = (contentId) => {
    if (monsterContentSettings.value.includes(contentId)) {
        if (monsterContentSettings.value.length > 1) {
            monsterContentSettings.value = monsterContentSettings.value.filter((id) => id !== contentId);
        }
        else {
            toast.add({
                severity: "error",
        ...
        monsterContentSettings.value = [...monsterContentSettings.value, contentId];
    }
};
...
if (props.noCard) {
    ...
    for (const [content] of __VLS_getVForSourceType((__VLS_ctx.contentStore.getAllWithMonsters()))) {
        ...
        __VLS_asFunctionalElement(__VLS_intrinsicElements.div, __VLS_intrinsicElements.div)({
            ...
            ...{ class: ({
                    selected: __VLS_ctx.monsterContentSettings.includes(content.id),
                    unselected: !__VLS_ctx.monsterContentSettings.includes(content.id)
                }) },
