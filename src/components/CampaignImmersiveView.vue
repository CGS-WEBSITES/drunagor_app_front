<template>
    <div class="immersive-container" :class="{ 'desktop-layout': $vuetify.display.mdAndUp }">
        <div class="map-viewport" ref="mapContainerRef" @mousedown="startDrag" @touchstart="startDrag"
            @wheel.prevent="handleZoom">
            <div class="map-content" :style="mapTransformStyle">
                <img v-if="currentBackgroundImage" :src="currentBackgroundImage" class="map-image" alt="Campaign Map"
        ...
        <div class="hud-layer">
            <div class="hud-area top-left">
                <div class="interactive-content d-flex flex-column align-start">
                    <div class="objective-panel mb-2">
                        <div class="objective-label text-uppercase text-caption font-weight-bold text-blue-lighten-3">
                            Current Objective:
                        </div>
                    ...
                    <v-menu :close-on-content-click="false" v-if="showSaveCampaignButton">
                        <template v-slot:activator="{ props }">
                            <v-btn v-bind="props" icon="mdi-cog" variant="text" density="compact" color="white"
                                class="opacity-50 hud-icon-btn ml-1 mb-2"></v-btn>
                        </template>
                    ...
                    <div class="d-flex flex-column gap-2 mt-1">
                        <v-tooltip text="Campaign Book" location="right">
                            <template v-slot:activator="{ props }">
                                <div v-bind="props" class="bookmark-tab left-side" @click.stop="openBookDialog">
                                    ...
                                    <span
                                        class="d-none d-md-inline font-weight-bold text-caption text-label ml-2">BOOKS</span>
                                </div>
                            </template>
                        </v-tooltip>
                        ...
                        <v-tooltip text="Keywords" location="right">
                            <template v-slot:activator="{ props }">
                                <div v-bind="props" class="bookmark-tab left-side" @click.stop="openKeywordsDialog">
                                    ...
                                    <span
                                        class="d-none d-md-inline font-weight-bold text-caption text-label ml-2">KEYWORDS</span>
                                </div>
                            </template>
                        </v-tooltip>
                        ...
                        <v-tooltip text="Door Instructions" location="right">
                            <template v-slot:activator="{ props }">
                                <div v-bind="props" class="bookmark-tab left-side blue-border-tab"
                                    @click.stop="openOnlyInstructions">
                                    <img src="@/assets/door.png" alt="Door" class="tab-icon-img"
                                        style="width: 24px; height: 24px" />
                                    <span
                                        class="d-none d-md-inline font-weight-bold text-caption text-label ml-2">RULES</span>
                                </div>
                            </template>
                        </v-tooltip>
                    </div>
                ...
                <div class="interactive-content d-flex flex-column align-end gap-2">
                    <div class="d-flex flex-row align-center gap-2 mb-1">
                        <v-tooltip text="Back to Dashboard" location="bottom">
                            <template v-slot:activator="{ props }">
                                <v-btn v-bind="props" icon="mdi-arrow-left" class="square-hud-btn"
                                    @click="goBack"></v-btn>
                            </template>
                        </v-tooltip>
                    </div>
                    ...
                    <div v-if="currentMonsters.length > 0" class="d-flex flex-column align-end mt-2 w-100">
                        <div class="bookmark-tab right-side mb-1" @click="showMonstersPanel = !showMonstersPanel">
                            <span
                                class="text-caption font-weight-bold mr-2 text-grey-lighten-1 d-none d-md-inline">MONSTERS</span>
                            <v-icon size="small">{{
                        ...
                        <transition-group name="slide-fade" tag="div"
                            ...
                            <div v-for="(monster, index) in currentMonsters" :key="monster + index" class="monster-card"
                ...
                <div class="interactive-content d-flex flex-column align-start gap-2">
                    <div class="d-flex gap-2">
                        <v-tooltip text="Player List" location="top">
                            <template v-slot:activator="{ props }">
                                <v-btn v-bind="props" icon="mdi-account-group" class="square-hud-btn"
                                    @click.stop="playerListDialogVisible = true"></v-btn>
                            </template>
                        </v-tooltip>

                        <v-tooltip text="Leave Campaign" location="top" v-if="showSaveCampaignButton">
                            <template v-slot:activator="{ props }">
                                <v-btn v-bind="props" icon="mdi-logout" class="square-hud-btn" color="error"
                                    @click.stop="confirmLeave"></v-btn>
                            </template>
                        </v-tooltip>
                    </div>
                </div>
            ...
            <div class="hud-area bottom-center">
                <div class="heroes-rack interactive-content">
                    <div v-for="hero in enrichedHeroes" :key="hero.heroId" class="hero-token-wrapper"
                        @click.stop="openHeroCard(hero)">
                        <div class="hero-token">
                            <v-img :src="hero.images?.avatar ||
                                hero.images?.trackerimage ||
                                '/assets/hero/avatar/default.webp'
                                " cover class="hero-token-img" @error="onImgError"></v-img>
                        </div>
                        <div class="hero-name-tag">{{ hero.name }}</div>
                    </div>
                    <div v-if="showSaveCampaignButton && enrichedHeroes.length < 4" class="hero-token-wrapper add-hero"
            ...
            <div class="hud-area bottom-right">
                <div class="interactive-content d-flex flex-column align-end gap-3">
                    <v-tooltip text="Interactions" location="left" v-if="showInteractionsButton">
                        <template v-slot:activator="{ props }">
                            <div v-bind="props" class="right-tab-btn interaction-tab"
                    ...
                    <v-tooltip text="Read Scene" location="left">
                        <template v-slot:activator="{ props }">
                            <div v-bind="props" class="right-tab-btn grey-tab" @click.stop="readTheScene">
                    ...
                    <v-tooltip :text="nextButtonLabel" location="left">
                        <template v-slot:activator="{ props }">
                            <div v-bind="props" class="right-tab-btn primary-tab" style="
        ...
        <v-dialog v-model="interactionsDialog.visible" fullscreen transition="dialog-bottom-transition" :scrim="false">
            <v-card color="black">
                <InteractViewNew v-if="interactionsDialog.visible" :current-door="activeCampaignData.door"
                    :wing="activeCampaignData.wing" @close="interactionsDialog.visible = false" />
            </v-card>
        </v-dialog>
        ...
        <v-dialog v-model="monsterGroupDialog.visible" max-width="1000" scrollable>
            <v-card class="bg-grey-darken-4 rounded-xl border-thin">
                <v-toolbar color="rgba(0,0,0,0.8)" density="compact">
                    <v-toolbar-title class="cinzel-font text-red-accent-2 font-weight-bold">ENEMIES IN
                        ROOM</v-toolbar-title>
                    <v-spacer></v-spacer>
                    <v-btn icon="mdi-close" @click="monsterGroupDialog.visible = false"></v-btn>
                </v-toolbar>
                <v-card-text class="pa-4">
                    <v-row justify="center" align="center">
                        <v-col v-for="monster in currentMonsters" :key="monster" cols="4" md="6" lg="4"
                            class="d-flex justify-center">
                            <div class="d-flex flex-column align-center">
                                <img :src="getMonsterImageSrc(monster)"
        ...
        <v-dialog v-model="doorScannerDialog.visible" max-width="500" persistent>
            <v-card class="bg-grey-darken-3 rounded-xl overflow-hidden">
                ...
                <NextDoorQRScanner v-if="doorScannerDialog.visible" @scanned="handleDoorScanned"
        ...
        <v-dialog v-model="leaveDialog.visible" max-width="400">
            <v-card title="Leave Campaign" class="bg-grey-darken-3">
                <v-card-text class="pa-4 text-body-1">Are you sure you want to permanently leave this
                    campaign?</v-card-text>
                <v-card-actions><v-spacer /><v-btn color="grey"
                        @click="leaveDialog.visible = false">Cancel</v-btn><v-btn color="error" variant="elevated"
                        @click="leaveDialog.onConfirm">Leave</v-btn></v-card-actions>
            </v-card>
        </v-dialog>
        ...
        <v-dialog v-model="bossConfirmationDialog.visible" max-width="400" persistent>
            <v-card class="bg-red-darken-4 border-xl border-white rounded-lg">
                ...
                <v-card-actions class="justify-center pb-4">
                    ...
                    <v-btn color="black" class="text-red-accent-2 font-weight-bold" variant="elevated"
                        @click="confirmBossStart">FIGHT!</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
        ...
        <v-dialog v-model="narrativeDialogVisible" max-width="800" scrollable persistent>
            <v-card class="book-style-card rounded-xl overflow-hidden">
                <v-toolbar color="#10594f" density="compact" class="px-2">
                    <v-toolbar-title class="text-white font-weight-bold pl-2" style="font-family: serif">{{
                        currentDoorInstruction?.title?.toUpperCase() }} -
                        STORY</v-toolbar-title>
                    <v-spacer></v-spacer>
                    <v-btn icon="mdi-close" variant="text" color="white"
                        @click="narrativeDialogVisible = false"></v-btn>
                </v-toolbar>
                ...
                <v-card-actions class="justify-center py-4" style="background-color: #eee8e0">
                    <v-btn color="brown-darken-3" variant="flat" size="large" class="px-6 font-weight-bold"
                        @click="proceedToInstructions">CONTINUE TO INSTRUCTIONS
        ...
        <v-dialog v-model="instructionsDialogVisible" max-width="900" scrollable>
            <v-card class="book-style-card rounded-xl overflow-hidden">
                <v-toolbar color="#10594f" density="compact" class="px-2">
                    ...
                    <v-spacer></v-spacer>
                    <v-btn icon="mdi-close" variant="text" color="white"
                        @click="instructionsDialogVisible = false"></v-btn>
                </v-toolbar>
                <v-card-text class="pa-4" style="max-height: 80vh; overflow-y: auto">
                    <v-container fluid v-if="currentDoorInstruction">
                        <v-row><v-col cols="12">
            ...
            <v-card class="bg-grey-darken-4 rounded-xl hero-detail-card" v-if="heroCardDialog.hero">
                ...
                <v-card-text class="pa-0">
                    <div class="hero-tracker-header">
                        <v-img :src="heroCardDialog.hero.images.trackerInfo ||
                            heroCardDialog.hero.images.background
                            " width="100%" max-height="300" cover></v-img>
                    </div>
                    <v-container fluid class="pa-4">
                        <HeroDetailSummary :campaign-id="campaignId"
                            :hero-id="heroCardDialog.hero.heroId || heroCardDialog.hero.id" class="mb-4" />
                        <v-divider class="my-4 border-opacity-25"></v-divider>
                        <CampaignLogSequentialAdventure :campaign-id="campaignId" :hero="heroCardDialog.hero"
                            :hide-manage-button="true" />
                    </v-container>
                </v-card-text>
                <v-card-actions class="bg-grey-darken-3 pa-4 d-flex justify-space-between gap-4">
                    <v-btn color="amber-accent-4" variant="elevated" class="flex-grow-1 text-black font-weight-bold"
                        prepend-icon="mdi-sack" @click="
                            openResources(
                    ...
                    <v-btn color="light-blue-accent-3" variant="elevated"
                        class="flex-grow-1 text-black font-weight-bold" prepend-icon="mdi-shield-sword" @click="
                            openEquipment(
                                heroCardDialog.hero.heroId || heroCardDialog.hero.id,
                            )
                            ">
        ...
        <v-dialog v-model="playerListDialogVisible" max-width="600"><v-card title="Player List"
                class="bg-grey-darken-3"><v-card-text>
                    <CampaignPlayerList :campaign-id="campaignId" :show-remove-button="showSaveCampaignButton" />
                </v-card-text><v-card-actions class="d-flex flex-wrap justify-space-around pa-4"><v-btn
                        @click="openInviteDialog" variant="elevated" rounded color="primary">Invite Player</v-btn><v-btn
                        variant="text"
                        @click="playerListDialogVisible = false">Close</v-btn></v-card-actions></v-card></v-dialog>
        <v-dialog v-model="addHeroDialogVisible" max-width="500"><v-card title="Add Hero"
                class="bg-grey-darken-3"><v-card-text>
                ...
                </v-card-text><v-card-actions><v-btn block
                        @click="addHeroDialogVisible = false">Close</v-btn></v-card-actions></v-card></v-dialog>
        ...
        <v-dialog v-model="wing4ChoiceDialog.visible" max-width="400">
            <v-card class="bg-grey-darken-3 pa-4 text-center">
                ...
                <v-card-actions class="justify-center flex-column gap-2">
                    <v-btn block color="purple-accent-2" variant="elevated" class="text-black font-weight-bold"
                        @click="commitWing4Choice('DRACONIC CHAPEL')">
                        <v-icon start>mdi-church</v-icon> Door 1
                    </v-btn>
                    <v-btn block color="cyan-accent-2" variant="elevated" class="text-black font-weight-bold"
...
const saveDoorOpening = async (doorCode: string): Promise<boolean> => {
    ...
    try {
        ...
        console.log(`[ImmersiveView] Door saved: ${doorObj.name}`);
        return true;
    } catch (error: any) {
        // 409 = already exists, not an error
        if (error.response?.status === 409) {
            console.log(`[ImmersiveView] Door already opened: ${doorObj.name}`);
            return true;
        }
...
// QR Code to Door mapping - using book prefix
...
async function handleDoorScanned(code: string) {
    ...
    // Save the opened door to database
