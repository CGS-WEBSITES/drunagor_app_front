      ...
      <v-card class="mobile-menu-card">
        ...
        <v-list class="mobile-menu-list" v-model:opened="openGroups">
          <v-list-subheader class="font-weight-bold text-uppercase mt-2"
            >Adventure Story</v-list-subheader
          >
          <v-list-group
          ...
          >
            ...
            >
              <template #prepend>
                <v-avatar size="24"
                  ><span class="text-caption">{{ index + 1 }}</span></v-avatar
                >
              </template>
          ...
          <v-list-subheader class="font-weight-bold text-uppercase"
            >Reference Library</v-list-subheader
          >
          <v-list-group
              ...
              >
                ...
                <v-container class="pa-0 pt-2 ml-3">
                  <v-row>
                    <v-col cols="12">
                      <div
                        v-for="(item, idx) in currentPage.content"
                        :key="idx"
                        :id="'content-' + idx"
                        class="content-block"
                      >
            ...
            <div v-else-if="currentAuxBook" :key="'aux-' + currentView">
              ...
              >
                <v-container class="pa-4">
                  <div
                    v-for="(chapter, cIdx) in currentAuxBook.chapters || [
                      {
                        chapterTitle: currentAuxBook.chapterTitle,
                        sections: currentAuxBook.mechanics,
                      },
                    ]"
                    :key="cIdx"
                  ...
                  >
                    <div class="header-banner" :style="headerBannerStyle">
                      ...
                      >
                        <h4 class="section-title">
                          {{ currentAuxBook.pageTitle }}
                        </h4>
                      </div>
                      <h2 class="chapter-title-banner">
                        {{
                          chapter.chapterTitle || currentAuxBook.chapterTitle
                        }}
                      </h2>
                    </div>
                    ...
                    <div class="body-text-mechanics pa-4 mt-3">
                      <template v-if="chapter.tutorials">
                        <section
                          v-for="(t, tIdx) in chapter.tutorials"
                          :key="tIdx"
                          :id="'sec-' + cIdx + '-' + tIdx"
                          class="mb-4"
                        >
                      ...
                      <template v-else-if="chapter.sections">
                        <section
                          v-for="(s, sIdx) in chapter.sections"
                          :key="sIdx"
                          :id="'sec-' + cIdx + '-' + sIdx"
                          class="mb-4"
                        >
...
const headerBannerStyle = computed(() => {
  ...
  if (
    sectionName.includes("WING 3") ||
    sectionName.includes("WING 4") ||
    currentView.value === "dragon"
  ) {
...
.book-page {
  ...
  margin: 20px;
  box-shadow: 0 0 10px rgba(94, 69, 57, 0.3);
  border-radius: 12px;
  min-height: 400px;
}
.header-banner {
