  ...
  <v-container max-width="800" style="min-width: 360px;" class="pa-4">
    <v-card rounded="lg" elevation="3" color="primary" class="pl-1 pt-1 pr-1 pb-0">
      ...
      <v-card-title class="d-flex justify-space-between pb-0">
        ...
        <span class="text-uppercase text-bold text-h5 mb-4 pb-0">{{ userRewards.length }}</span>
      </v-card-title>
      ...
      <v-virtual-scroll class="pt-0" :items="userRewards" :item-height="130">
        <template #default="{ item }">
          <v-card
            rounded="lg"
            elevation="3"
            class="py-2 px-0 mb-3 d-flex align-center"
            color="secundary"
          >
            <v-row class="align-center">
              <!-- Ícone -->
              <v-col cols="2" md="2" class="d-flex align-center justify-center ml-3 ml-md-0">
                <v-img
                  :src="item.image"
                  alt="Reward Icon"
                  max-height="80"
