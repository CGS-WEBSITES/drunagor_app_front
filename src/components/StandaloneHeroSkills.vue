<template>
    ...
    <v-row no-gutters class="justify-space-around">
      ...
      >
        <div class="d-flex align-center justify-center mb-2" style="gap: 8px">
          ...
          <v-icon size="small" :color="skillTypeColors[skillType]"
            >mdi-cube</v-icon
          >
        </div>
        ...
        >
          <v-img
            :src="getSelectedSkillCard(skillType)!.image"
            cover
            alt="Skill"
          ></v-img>
        </v-card>
        ...
        <v-divider
          class="my-6"
          v-if="index < skillTypes.length - 0"
        ></v-divider>
      </v-col>
    </v-row>
  </template>
  ...
  <v-dialog v-model="isAddSkillModalVisible" max-width="700">
    <v-card>
      ...
      <v-card-text>
        <v-row>
          <v-col
            v-for="card in availableSkillsForModal"
            :key="card.id"
            cols="12"
          >
  ...
  <v-dialog v-model="isViewSkillModalVisible" max-width="500">
    <v-card v-if="skillToView">
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="error" variant="elevated" @click="deleteSkill">{{
          t("Delete")
        }}</v-btn>
...
const skillTypes: Array<"melee" | "ranged" | "agility" | "wisdom"> = [
  "melee",
  "ranged",
  "agility",
  "wisdom",
...
function getSelectedSkillCard(skillType: string): SkillCard | undefined {
  const foundSkillId = selectedSkills.value.find((id) =>
    id.includes(`-${skillType}-`),
  );
...
function selectSkill(cardToSelect: SkillCard) {
  const otherSkills = selectedSkills.value.filter(
    (id) => !id.includes(`-${cardToSelect.skillType}-`),
  );
